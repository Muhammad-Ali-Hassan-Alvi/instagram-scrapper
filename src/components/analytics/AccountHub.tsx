    ...
    <section className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {accounts.map((account) => {
          const summary = snapshot.accountSummaries.find((row) => row.username === account.username);
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className={ui.label}>Instagram</p>
                  <h3 className="mt-1 text-xl font-semibold text-slate-900">@{account.username}</h3>
                </div>
                <span
              ...
              <dl className="mt-4 grid grid-cols-3 gap-3 text-sm">
                <div>
                  ...
                  <dd className="mt-0.5 font-semibold text-slate-900">
                    ...
                    {expected > 0 && (
                      <span className="font-normal text-slate-500"> / {formatNumber(expected)}</span>
                    )}
                  </dd>
                </div>
                <div>
